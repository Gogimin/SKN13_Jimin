...
SELECT emp_name, CHAR_LENGTH(emp_name) AS name_length
FROM EMP
...
select  emp_id,
        ...
        ceil(salary * 1.15) as "SAL_RAISE_올림",
        floor(salary * 1.15) as "SAL_RAISE_내림"
from 	emp;
...
select  emp_id,
        ...
        round(salary, -3), # 0을 기준으로 - 면 정수부분으로 + 면 소수부분(실수부분)으로
        round(salary, 3)
from    emp
...
select curdate();	-- date
select curtime();	-- time
-- insert into member(id, ... , join)date) value(?, ?, ... , curdate()) (# 이거 뭔지 못들음)
...
select  curdate(), -- date type
		...
		month(curdate()),
		day(curdate()),
		dayofweek(curdate()); # 일 월 화 수 목 금 토 / 1 2 3 4 5 6 7
...
***********************************************/
select '10' + '20'; # mysql이 알아서 정수형으로 변환후 덧셈
...
************************************* */
select curdate(); # 날짜 출력
select dayofweek(curdate()); # 요일을 숫자로 출력

select
case dayofweek(curdate()) when 1 then '일요일'
						  when 2 then '월요일'
                          when 3 then '화요일'
                          when 4 then '수요일'
                          when 5 then '목요일'
