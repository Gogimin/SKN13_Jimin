...
select  d.loc , d.dept_id, d.dept_name,
...
-- 부서가 업슨ㄴ 직원의 정보도 나오도록 조회.(**모든 직원**정보 조회) dept_name의 내림차선으로
select * from emp where dept_id is null;
select count(*) from emp;
select count(*)
from emp e join dept d on e.dept_id = d.dept_id;
# dept_id 가 null 인 정보(5명)는 다 빠진 상태로 조회.

select count(*)
from emp e left join dept d on e.dept_id = d.dept_id;
# emp에 있는 테이블이 모두 조회되기 때문에 107개가 그대로 나옴.
...
select * from emp where emp_id = 210; # 210번인 애는 null(값이 없는데)인데 1개로 나옴
...
select  e.emp_id , e.emp_name , e.salary , e.hire_date, 
		j.job_title,
        m.emp_name as '상사 이름', m.hire_date as '상사 입사일',
        d.dept_name, d.loc
from emp e  left join job j on e.job_id = j.job_id
