...
select job_id from emp where emp_id =120;    -- 1번

select emp_id, emp_name, job_id, salary      -- 2번
from emp
where job_id = 'ST_MAN'; -- 120번 직원의 job_id

select emp_id, emp_name, job_id, salary      -- 한번에 한거
from emp
...
# select emp_id, emp_name, salary
# from   emp
# where  salary > (IT PROG 부서직원 중 가장 높은 급여);
...
select e.emp_id, e.emp_name, e.job_id, year(e.hire_date) as hire_year, e.salary
from   emp e
where  e.mgr_id = (select emp_id from emp where emp_name = 'Alexander');
...
-- 중복된 데이터 조회 (emp 에서 여러번 나오는 이름을 조회)
select emp_name, cnt
from (
